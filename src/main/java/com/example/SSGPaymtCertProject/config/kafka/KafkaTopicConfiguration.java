 ...
 * autoCreate 를 false 로 하면 토픽이 자동 생성되지 않고 해당 클래스 설정처럼 토픽을 정의해줘야 한다.
 * 디폴트인 true 로 두면 토픽이 생성 로직이 자동 실행된다.
 ...
 * 토픽의 정보를 가지고와서 이미 존재하지 않는 객체(토픽)면 ExecutionException 발생한다.
...
public class KafkaTopicConfiguration {
    ...
    public KafkaAdmin.NewTopics ssgPaymtCertTopic() {
        return new KafkaAdmin.NewTopics(
                ...
                TopicBuilder.name("offsetTopic-listener").build(),
